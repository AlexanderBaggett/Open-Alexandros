...
def run_validation(
    ...
    generator = seeded_generator(torch, int(args.seed) + 1_000_003 + step, device)
    with evaluating(model, torch):
        for _ in range(args.val_batches):
            ...
            if out.loss is None:
                raise ValueError("validation forward did not return a loss")
...
def main() -> None:
    parser = make_arg_parser("Run a smoke causal-LM training loop.")
    ...
    trainability = apply_trainability(model, phase="ar", scope=args.trainable_scope)
    opt_params = list(trainable_parameters(model))
    if not opt_params:
        raise ValueError("AR training requires at least one trainable parameter")
    ...
    objective_fields = objective_log_fields("ar")
    checkpoint_metadata = phase_checkpoint_metadata("ar", trainability)
    ...
    if args.out:
        model.save_pretrained(Path(args.out), checkpoint_metadata=checkpoint_metadata)
